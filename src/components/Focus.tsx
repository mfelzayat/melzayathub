...
export function Focus() {
  return (
    <section id="focus" className="scroll-mt-24 px-5 py-20 md:px-10 md:py-24 lg:px-14">
      <div className="mx-auto max-w-[1400px]">
        <Reveal className="mb-10 flex flex-col gap-4 md:mb-12 md:flex-row md:items-end md:justify-between">
          ...
          <p className="max-w-sm text-sm leading-relaxed text-mute md:text-right">
            Fintech, healthcare, education, tourism. Markets that move people and money.
          </p>
        </Reveal>
        ...
        <Reveal>
          <ul className="flex flex-col border-y border-line sm:flex-row sm:divide-x sm:divide-line">
            {areas.map((title, i) => (
              <li
                key={title}
                className="flex flex-1 items-baseline justify-between gap-4 border-b border-line px-0 py-6 last:border-b-0 sm:flex-col sm:justify-center sm:gap-3 sm:border-b-0 sm:px-6 sm:py-9 md:px-8"
              >
